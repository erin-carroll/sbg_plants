...
import {
  ...
  Tooltip, IconButton, Drawer, useMediaQuery, useTheme,
} from '@mui/material';
import {
      ...
      <Box sx={{ display: 'flex', flex: 1, overflow: 'hidden', mt: '56px', maxWidth: { xl: 1920 }, mx: 'auto', width: '100%' }}>
        ...
        {isMobile && (
          <Drawer
            ...
            PaperProps={{ sx: { width: 300, mt: '56px', height: 'calc(100% - 56px)' } }}
          >
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, px: 2, pb: 2, pt: 2, flex: 1, overflowY: 'auto' }}>
              ...
              <Button variant="contained"
                ...
                onClick={() => { q.handleApply(); setMobileFilterOpen(false); }}
                disabled={q.loading} fullWidth>Apply</Button>
              <Button variant="contained" color="secondary" startIcon={<ResetIcon />}
                onClick={() => { q.handleReset(); job.reset(); spectra.reset(); setRunDisabled(false); setExtractDisabled(false); clearDrawnRef?.current?.(); }}
                disabled={q.loading} fullWidth>Reset</Button>
              {hasResults && <Typography variant="body2" color="text.secondary">{q.totalPlots} plots matched</Typography>}
            </Box>
          </Drawer>
        )}
        ...
        {!isMobile && (
          ...
          >
            ...
            {!filterCollapsed && (
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, px: 2, pb: 2, flex: 1, overflowY: 'auto' }}>
                <StagingToggle onToggle={() => { q.handleReset(); job.reset(); spectra.reset(); setRunDisabled(false); setExtractDisabled(false); clearDrawnRef?.current?.(); }} />
                <LinkedFilterPanel
                  campaignName={q.campaignName}
                  setCampaignName={q.setCampaignName}
                ...
                <Button variant="contained"
                  startIcon={q.loading ? <CircularProgress size={16} color="inherit" /> : <SearchIcon />}
                  onClick={q.handleApply} disabled={q.loading} fullWidth>Apply</Button>
                <Button variant="contained" color="secondary" startIcon={<ResetIcon />}
                  onClick={() => { q.handleReset(); job.reset(); spectra.reset(); setRunDisabled(false); setExtractDisabled(false); clearDrawnRef?.current?.(); }}
                  disabled={q.loading} fullWidth>Reset</Button>
                {hasResults && <Typography variant="body2" color="text.secondary">{q.totalPlots} plots matched</Typography>}
        ...
        {/* Right — main content */}
        <Box sx={{ flex: 1, overflowY: 'auto', p: { xs: 1, sm: 2 }, display: 'flex', flexDirection: 'column', gap: 2, position: 'relative', minWidth: 0 }}>

          {/* Mobile filter toggle */}
          {isMobile && (
            <Button variant="outlined" startIcon={<FilterIcon />}
              onClick={() => setMobileFilterOpen(true)} size="small" sx={{ alignSelf: 'flex-start' }}>
              Filters{hasResults ? ` · ${q.totalPlots} plots` : ''}
            </Button>
          )}
          ...
          {q.error && <Alert severity="error" onClose={() => q.setError(null)}>{q.error}</Alert>}
          ...
          {q.loading && <Box sx={{ display: 'flex', justifyContent: 'center', pt: 4 }}><CircularProgress /></Box>}
          {!q.loading && q.hasQueried && q.totalPlots === 0 && <Alert severity="info">No plots matched your filters.</Alert>}
                ...
                <Paper elevation={1} sx={{ px: { xs: 1, sm: 2 }, py: 1.5 }}>
                  <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
                    <Button size="small" variant="outlined" startIcon={<PrevIcon />}
                      onClick={q.handlePrev} disabled={!hasPrev || q.loading}>Prev</Button>
                    ...
                    <Button size="small" variant="outlined" endIcon={<NextIcon />}
                      onClick={q.handleNext} disabled={!hasNext || q.loading}>Next</Button>
                  </Stack>
                  <Stack direction="row" alignItems="center" spacing={1} flexWrap="wrap" useFlexGap sx={{ gap: 1 }}>
                    {q.hasQueried && (
                      <Typography variant="caption" color="text.secondary" sx={{ whiteSpace: 'nowrap' }}>{pixelLabel}</Typography>
                    )}
                    <Box sx={{ flex: 1 }} />
                    <ToggleButtonGroup value={spectra.spectraType} exclusive
                      onChange={(_, v) => { if (v) spectra.setSpectraType(v); }} size="small">
